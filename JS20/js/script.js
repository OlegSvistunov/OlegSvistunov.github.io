...
(function() {
    ...
    var skills = _.sortBy(_.union(_.flatten(_.map(data, "skills"))));
    console.log("skills", skills);
    // alert(skills)

    var nameArrays = _.map(data, function(jsonData) {
        return {
            "name": jsonData.name,
    ...
    var names  = _.map(_.sortBy(nameArrays, "friends"), "name");
    console.log("name", names);
    // alert(names)

    var friends = _.sortedUniq(_.map(_.sortBy(_.flatten(_.map(data, "friends")), "name"), "name"));
